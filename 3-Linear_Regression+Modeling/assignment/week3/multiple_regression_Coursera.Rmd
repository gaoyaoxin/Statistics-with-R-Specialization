...
$\hat{score} = 0.067\times bty_avg + 3.880$

The intercept in this case refers to the students that rate the professor with a `bty_avg` of 0, would give the professor a final score of 3.880 or either 3 or 4 (Since only discrete values are allowed).
...
Yes, `bty_avg` is a significant predictor of score. Furthermore, the addition of `gender` has reduced the p-value of `bty_avg`.
...
the evaluation score for a professor, Dr. Hypo Thetical, who is a male tenure 
track professor with an average beauty of 3.
...
Multicollinearity reduces the precision of the estimate coefficients, which weakens the statistical power of your regression model. You might not be able to trust the p-values to identify independent variables that are statistically significant. [Statisticsbyjim](https://statisticsbyjim.com/regression/multicollinearity-in-regression-analysis/)

Since the p-values and the estimates didn't change by an order of magnitude greater than $10^{-1}$, the `gender` variable doesn't introduce collinearity.
...
* With courses, we'll be capturing on different attributes such as number of projects, assignments, etc.
* But, class courses are independent of each other so evaluation scores from one course is independent of the other even if the course is being taught by the same professor.
* However, with regards to the conditions of linear regression, the impact should be negligible, if in case the same columns/features are used.
...
`age`            | low age
`cls_perc_eval`  | high percent of students in class who completed evaluation.
`cls_did_eval`   | high number of students in class who completed evaluation.
`cls_students`   | high total number of students in class.
`cls_level`      | upper.
`cls_profs`      | multiple.
`cls_credits`    | one credit (lab, PE, etc.)
`bty_avg`        | high
`pic_outfit`     | formal.
...
No, since this is only an observational study and because the $R^{2}$ value is very low, suggesting a weak association.
