...
We use a scatter plot to display relationships between two numerical variables.
...
corr_coeff
```
When evaluating whether a model is linear, the following conditions should be met:
...
The relationship between `at_bats` and `runs` looks linear. The correlation coefficient of 0.61 confirms a weak linear association between the two variables.

#### 2. Nearly Normal Residuals:

Source: [Quick start guide - R software and data visualization](http://www.sthda.com/english/wiki/ggplot2-histogram-plot-quick-start-guide-r-software-and-data-visualization)
...
We can confirm this with a residual plot. There is a constant variability, hence this is homoscedasticity.. However, one of the potential outliers is a team with approximately 5520 at bats.
...
Since `bat_avg` has the highest $R^{2}$ value and the lowest p_value, we can say it's the best predictor of `runs`.
...
Since `new_obs` has the highest $R^{2}$ value and the lowest p_value, we can say it's the best predictor of `runs`.
...
Yes, since the p_value for the intercept is very low, we reject the null hypothesis of no relationship (i.e. slope of 0). This means the explanatory variable is a significant predictor of the response variable. Furthermore, the p_value of the F-Statistic is also very low. 
