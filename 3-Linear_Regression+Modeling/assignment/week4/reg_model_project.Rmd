...
Based on the boxplot above, the ratings are most variable for the Documentary category and least variable for the TV Movie category.
...
All else held constant, the model predicts that:
   - the "Animation" genre gets a rating which is 0.25 points lower than all other genres, on average.
   - the "Art House & International" genre gets a rating which is 0.7 points higher than all other genres, on average.
   - the "Comedy" genre gets a rating which is 0.05 points lower than all other genres, on average.
   - the "Drama" genre gets a rating which is 0.61 points higher than all other genres, on average.
   - the "Horror" genre gets a rating which is 0.09 points lower than all other genres, on average.
   - the "Musical & Performing Arts" genre gets a rating which is 0.95 points higher than all other genres, on average.
   - the "Mystery & Suspense" genre gets a rating which is 0.39 points higher than all other genres, on average.
   - the "Other" genre gets a rating which is 0.71 points higher than all other genres, on average.
   - the "Science Fiction & Fantasy" genre gets a rating which is 0.001 points higher than all other genres, on average.
   
#### runtime

The model predicts, given that all else is held constant, for each 1-minute increase in runtime, the imdb_rating increases by 0.01 points. Even though the predictor is significant according to its p-value, it doesn't necessarily mean that a longer movie is better. The model also predicts that every single minute only contributes to a minor increase in rating. Furthermore, in reality, a longer or shorter movie doesn't result in a popular film, it's how "engaging" or "engrossing" a film is, a highly subjective variable to measure. Moreover, it's difficult to determine the "engaging-ness" of a title prior to its release.
