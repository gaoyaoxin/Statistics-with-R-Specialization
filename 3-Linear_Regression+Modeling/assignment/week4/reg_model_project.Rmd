...
```{r}
lm1 <- lm(imdb_rating ~ title_type + genre + runtime + mpaa_rating + studio
          + dvd_rel_month + dvd_rel_day + best_actor_win + best_actress_win
          + thtr_rel_year + thtr_rel_month + thtr_rel_day + best_dir_win, data = movies)
sum_lm1 <- summary(lm1)
tail(sum_lm1$coefficients[,4])
```

#### 3. Repeat until all remaining variables are significant

The next highest p-value is that of `dvd_rel_day`, so we'll be eliminating it and re-fitting our model.

```{r}
lm2 <- lm(imdb_rating ~ title_type + genre + runtime + mpaa_rating + studio
          + dvd_rel_month + best_actor_win + best_actress_win
          + thtr_rel_year + thtr_rel_month + thtr_rel_day + best_dir_win, data = movies)

tail(summary(lm2)$coefficients[,4])
```

Rather than displaying each summary, this project will be 

```{r}
# removing best_actor_win
lm3 <- lm(imdb_rating ~ title_type + genre + runtime + mpaa_rating + studio
          + dvd_rel_month + best_actress_win
          + thtr_rel_year + thtr_rel_month + thtr_rel_day + best_dir_win, data = movies)
tail(summary(lm3)$coefficients[,4])

# removing best_actress_win
lm4 <- lm(imdb_rating ~ title_type + genre + runtime + mpaa_rating + studio
          + dvd_rel_month + thtr_rel_year + thtr_rel_month + thtr_rel_day 
          + best_dir_win, data = movies)
tail(summary(lm4)$coefficients[,4])

# removing thtr_rel_day
lm5 <- lm(imdb_rating ~ title_type + genre + runtime + mpaa_rating + studio
          + dvd_rel_month + thtr_rel_year + thtr_rel_month 
          + best_dir_win, data = movies)
tail(summary(lm5)$coefficients[,4])

# removing thtr_rel_year
lm6 <- lm(imdb_rating ~ title_type + genre + runtime 
          + mpaa_rating + studio + dvd_rel_month + thtr_rel_month 
          + best_dir_win, data = movies)
tail(summary(lm6)$coefficients[,4])

# removing thtr_rel_month
lm7 <- lm(imdb_rating ~ title_type + genre + runtime 
          + mpaa_rating + studio + dvd_rel_month 
          + best_dir_win, data = movies)
tail(summary(lm7)$coefficients[,4])

# removing studio
lm8 <- lm(imdb_rating ~ title_type + genre + runtime 
          + mpaa_rating + dvd_rel_month 
          + best_dir_win, data = movies)
summary(lm8)
```

Since the p-value for the entire value is below 0.05, the model as a whole is significant. This also means that at least one of the predictors are significant, conditional on the other variables included in the model.
