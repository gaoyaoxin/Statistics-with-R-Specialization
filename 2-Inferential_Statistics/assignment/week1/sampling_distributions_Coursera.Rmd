...
<li> __50\% of houses in Ames are smaller than 1,499.69 square feet.__ </li>
...
__Answer:__ The mean of the sample distribution is 1628.22 with a standard deviation of 777.84. The middle 50% of the data lies between 1048.25 and 1854.5. Since the sample median is 1410.50, which is much smaller than the sample mean, we can say that the distribution is right skewed. Concretely, the histogram also emphasizes the fact that this sample distribution is right skewed.
...
<li> __Sample size of 1000.__ </li>
</ol>
...
ggplot(data = sample_means50, aes(x = x_bar)) +
  geom_histogram(binwidth = 20) + 
  geom_vline(xintercept = mean(sample_means50$x_bar), color='red')
sample_means50 %>%
  summarise(n = n())
```
_Answer:_ Since this is a sampling distribution which meets the conditions set by the Central Limit Theorem, we would expect the distribution to be normally distributed. As shown by the histogram above, this is the case. The distribution is centered around 1500, which is very close to the population mean.
...
[Source](https://stackoverflow.com/questions/42676348/multiple-random-sampling-in-r)

```{r practice-sampling-dist}
# type your code for the Exercise here, and Run Document
sample_means_small <- do.call('rbind', lapply(c(1:25), function(x){
  ames %>% 
    sample_n(10) %>%
...
```
__Answer:__ There are 25 observations in this dataframe, with each row or observation representing mean of a random sample of 10 house areas.
...
<ol>
<li> __Each element represents a mean square footage from a simple random sample of 10 houses.__ </li>
...
__Answer:__ Each observation of a sampling distribution is the mean of a area of a sample distribution. A sampling distribution displays the means of sample distributions of size `n`. As the sample size increases:
* the mean moves closer to the true population mean
* the standard error decreases
* the sampling distribution tends to normalcy

When increasing the number of samples, the mean, standard error and the shape almost remains the same.
</div>
...
sample_means50 %>%
  summarise(x_bar = mean(x_bar))
```
__Answer:__ The shape of the distribution is symmetric and unimodal. The mean home price = 180634.5
...
sample_means150 %>%
  summarise(x_bar = mean(x_bar))
```
__Answer:__ 180639.7

6. Which of the following is false? 
<ol>
<li> __The variability of the sampling distribution with the smaller sample size (`sample_means50`) is smaller than the variability of the sampling distribution with the larger sample size (`sample_means150`).__ </li>
...
sample_means50 %>%
  summarise(mu50 = mean(x_bar), SE50 = sd(x_bar)/(n()^0.5))
sample_means150 %>%
  summarise(mu150 = mean(x_bar), SE150 = sd(x_bar)/(n()^0.5))
```
