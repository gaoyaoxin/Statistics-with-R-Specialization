...
<li> __The distribution has several extreme values on the right side.__ </li>
...
sfo_feb_flights %>% 
  summarise(mean(arr_delay), sd(arr_delay), median(arr_delay), IQR(arr_delay))
```
...
nycflights %>%
  group_by(month) %>%
  summarize(med_dd = median(dep_delay)) %>%
  arrange(desc(med_dd)) %>%
  sample_n(5)
```
...
<li> __There is an overall positive association between distance and average speed.__</li> 
...
nycflights %>%
  ...
  summarize(arr_time_dep_delay = sum(arr_type == 'on time' & dep_type == 'delayed')/n() * 100)
```